...
do
  ...
  python3 ${phaseEXT} --input SetB_02/phaseExtendedHaplotype_SetB_02.txt --SOI ${item} --output SetB_02/phased_${item}_SetB_run02 --numHets 40 --lods 1 --writeLOD yes --hapStats yes --addMissingSites no

  # also write the path of the output directory for each sample
  # so, they can be merged later 
  echo "SetB_02/phased_${item}_SetB_run02/extended_haplotype_${item}.txt" >> files_to_merge_SetB_run02.txt
done
...
echo "$(tail -n +2 files_to_merge_SetB_run02.txt)" > files_to_merge_SetB_run02.txt
