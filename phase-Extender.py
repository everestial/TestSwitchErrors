...
def main():
    ...
    with open(input_file) as input_data, open(
        outputdir + "/" + "missingdata_" + soi + ".txt", "w"
    ) as missing_data:
        ...
        print(
            '  - Lines that have data missing for sample "%s" is written in the file "%s" '
        ...
        print()
        print(
            '# Filtered the lines that have data missing for sample "%s"; check the file "%s" '
            % (soi, missing_data.name)
        )
        ...
        for chr_, data_by_chr in good_data_by_group:
            pd.DataFrame.to_csv(
                data_by_chr,
                "chunked_Data_" + soi + "/" + soi + ":" + str(chr_),
                sep="\t",
                index=False,
                header=True,
            )
        ...
        initial_haplotype = None
        good_data = None
        input_file = None
        good_data_by_group = None
        samples = None
        input_data = None
        data_by_chr = None
        del (
...
def multiproc(sample_list, pool, hapstats):
    ...
    all_files_path = [
        (item, sample_list)
        for item in list(os.path.join(path, part) for part in os.listdir(path))
    ]
    ...
    # print(args)
    # breakpoint()
    # all_files_path_new = [{'file_path': x } for x in all_files_path] #+ [{'a':1}]
    data_parameters = []

    for x_path in all_files_path:
        temp_dict = dict()

        temp_dict["file_path"] = x_path
        temp_dict["soi"] = args.SOI
    ...
    print()
    print(
        "Completed haplotype extension for all the chromosomes."
...
def groupby_and_read(input_parameters):

    # global outputdir
    # global use_bed
    soi = input_parameters["soi"]
    snp_threshold = input_parameters["snpTh"]

    num_of_hets = int(input_parameters["numHets"])
    lods_cut_off = float(input_parameters["lods"])
    writelod = input_parameters["writeLOD"]
    maxed_as = "+" if input_parameters["culLH"] == "maxSum" else "*"
    # global snp_threshold
    # global num_of_hets
    # global lods_cut_off
    # global use_sample  # used in coherence with "sample_list"
    ...
    print("Maxed as ", maxed_as)
    # print('Groupby soi',soi)
    # good_data_by_contig = open(input_parameters[0], 'r')
    # print(input_parameters[0])
    # print(input_parameters[0][0])

    good_data_by_contig = open(input_parameters["file_path"][0], "r")
    chr_ = good_data_by_contig.name.split(":")[-1]
    # sample_list = input_parameters[1]
    sample_list = input_parameters["file_path"][1]

    contigs_group = pd.read_csv(StringIO(good_data_by_contig.read()), sep="\t")
    # breakpoint()
    # print(contigs_group)
    # print(type(contigs_group))
    print("test 01")
...
def process_consecutive_blocks(
    ...
    for (k1, v1), (k2, v2) in zip(
        grouped_data.items(), islice(grouped_data.items(), 1, None)
    ):
...
def compute_haplotype_stats(hap_data, soi, prefix):
    ...
    pd.DataFrame.to_csv(
        hap_stats,
        outputdir + "/" + prefix + "_haplotype_stats_" + soi + ".txt",
        sep="\t",
        header=True,
        index=False,
    )
    ...
    with open(
        outputdir + "/" + "total_vars_" + soi + "_" + prefix + ".png", "wb"
    ) as fig_initial:
    ...
    with open(
        outputdir + "/" + "total_haps_" + soi + "_" + prefix + ".png", "wb"
    ) as fig_initial:
    ...
    with open(
        outputdir + "/" + "hap_size_byVar_" + soi + "_" + prefix + ".png", "wb"
    ) as fig_initial:
        ...
        # plt.ylabel('frequency of the haplotypes')
        plt.suptitle(
            prefix + " histogram of size of the haplotype (by number of variants) \n"
            "for each chromosome"
        )
        plt.savefig(fig_initial)
    ...
    # make different plots for each chromosome, but X-axis is shared.
    with open(
        outputdir + "/" + "hap_size_byGenomicRange_" + soi + "_" + prefix + ".png", "wb"
    ) as fig_initial:
        ...
        # plt.ylabel('frequency of the haplotypes')
        plt.suptitle(
            prefix + " histogram of size of the haplotype (by Genomic Distance)\n"
            "for each chromosome"
        )
        plt.savefig(fig_initial)
