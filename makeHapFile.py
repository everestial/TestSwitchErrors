...
def main():
    ...
    print()
    print('Purpose of the application : This application simulates ReadBackPhased haplotype given *.haps, *.legend and *.sample file are provided.')
    
    parser.add_argument("-haps", help = 'the "*.haps" file that contains haplotype data in numeric format', required=True)
    parser.add_argument("-legend", help = 'the "*.legend" file that contains genomic coordinates and genotypes as IUPAC bases.', required=True)
    parser.add_argument("-sample_file", help = 'the "*.sample" file that contains sample names and have their position (rows) \
        matched with column position in "*.haps" file.', required=True)
    parser.add_argument("-chr", help = 'The chromosome name the "*.legend" and "*.haps" file belongs to.', required=True)
    parser.add_argument("-samples", help = 'The samples from "*.sample" file that are to be converted to RBphased data.', required=True)
    parser.add_argument("-output_truth", help = 'Write the truth HAPLOTYPE file.', required=True)
    parser.add_argument("-output_sim", help = 'Write the simulated HAPLOTYPE file.', required=True)
    ...
    else: sample_of_interest += args.sample
    ...
    with open(sample_f) as sample_file:
        ...
        if len(sample_of_interest) != 0:
            print('The following samples are not found in the provided "*.sample" file.')
    ...
    ref_alt_data.rename(index=str, columns={'position':'POS', 'a0':'REF', 'a1':'all-alleles'}, inplace=True)
    ...
    for sample_name, sample_idx in sample_pos_dict.items():
        ...
        haplotype[sample_name + '_hapA'] = haplotype.apply(lambda row: row['a0'] if row['hapA'] == 0 else row['a1'], axis = 1)
        haplotype[sample_name + '_hapB'] = haplotype.apply(lambda row: row['a0'] if row['hapB'] == 0 else row['a1'], axis = 1)
        
        # add the converted haplotype to column "sample_name:PG_al" 
        haplotype[sample_name + ':PG_al'] = \
            haplotype.apply(lambda row: row[sample_name + '_hapA'] 
            + '|' + row[sample_name + '_hapB'], axis=1) 
            
        # drop the not required columns
        haplotype.drop(['rsID', 'a0', 'a1', 'hapA', 'hapB', sample_name + '_hapA', sample_name + '_hapB'], 
            inplace=True, axis=1)
        ...
        hap_length = len(haplotype) # total number of "heterozyous varaints" in the haplotype 
        avg_rbphased_size = np.random.choice([5, 6, 7]) # randomly choosing avg_rbphased_size
        ...
        pi_index_values = [[x]*y for x, y in 
                            zip(unique_rbphased_index, rbphased_blocks)]    
        ...
        haplotype.insert(loc = 2, column = sample_name + ':PI', value=pd.Series(pi_index_values).values)
        
        ## Store the "truth haplotype" set to file 
        truth_haplotype = haplotype[['CHROM', 'POS', sample_name + ':PI', sample_name + ':PG_al']]
        ...
        haplotype[sample_name + ':PG_al'] = haplotype.apply(
            ...
            if row[sample_name + ':PI'] % 2 == 0 else row[sample_name + ':PG_al'], 
            axis = 1)
            
        ## Store the "simulated haplotype set" to a file. 
        simulated_haplotype = haplotype[['CHROM', 'POS', sample_name + ':PI', sample_name + ':PG_al']]
    ...
    for ks, vs in sample_hap_dict.items():         
        if len(truth_haplotype_file) == 0:
            ...
            truth_haplotype_file = truth_haplotype_file.merge(vs[0], on=['CHROM', 'POS'], how = 'outer').fillna('.')
            simulated_haplotype_file = simulated_haplotype_file.merge(vs[1], on=['CHROM', 'POS'], how = 'outer').fillna('.')


    ### Finally, merge the haplotype for each sample with "ref-alt" data to create complete HAPLOTYPE file
    final_haplotype_file = ref_alt_data.merge(truth_haplotype_file, on = ['CHROM', 'POS'], how = 'inner')
    ...
    final_simulated_haplotype_file = ref_alt_data.merge(simulated_haplotype_file, on = ['CHROM', 'POS'], how = 'inner')
