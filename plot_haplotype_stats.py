...
def compute_haplotype_stats(hap_data, soi, prefix, outputdir):
    ...
    plot_bar_hapstats(hap_stats,xcol= 'CHROM', ycol = 'total_Vars', filepath= var_path, xlabel= xlabel, ylabel = ylabel, suptitle= suptitle)
    ...
    plot_bar_hapstats(hap_stats,xcol= 'CHROM', ycol = 'total_haplotypes', filepath= hap_path, xlabel= xlabel, ylabel = ylabel, suptitle= suptitle)
    ...
    genomic_path = Path(outputdir , 'hap_size_byGenomicRange_' + soi + '_' + prefix + '.png')

    if len(hap_stats) == 1:
        plot_hist_one_chr(hap_stats, hist_by= 'num_Vars_by_PI', filepath= varsize_path, prefix= prefix)
        plot_hist_one_chr(hap_stats, hist_by= 'range_of_PI', filepath= genomic_path, prefix= prefix)

    else:
        plot_hist_multi_chr(hap_stats, hist_by= 'num_Vars_by_PI', filepath= varsize_path, prefix= prefix)
        plot_hist_multi_chr(hap_stats, hist_by= 'range_of_PI', filepath= genomic_path, prefix= prefix)
...
def plot_bar_hapstats(hap_stats,xcol, ycol, filepath, xlabel= None, ylabel = None, suptitle= None):
...
def plot_hist_one_chr(hap_stats, hist_by, filepath, prefix):
    ...
    sub_title = 'number of variants' if hist_by == 'num_Vars_by_PI' else 'Genomic Distance'
